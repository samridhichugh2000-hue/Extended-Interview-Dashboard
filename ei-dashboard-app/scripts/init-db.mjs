...
for (const stmt of [
  ...
  'ALTER TABLE employees ADD COLUMN mgr_feedback_details TEXT',
]) {
  try {
...
for (const stmt of [
  'ALTER TABLE employees ADD COLUMN ideas_count INTEGER',
  'ALTER TABLE employees ADD COLUMN ideas_details TEXT',
]) {
  try {
    await db.execute(stmt);
  } catch (err) {
