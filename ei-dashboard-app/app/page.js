...
import {
  EMP, STATUS, decorate, NAV, TITLES, PATHS, CARD_DEFS, STATUS_MAP, METRIC_HEADS,
  WORRY_BANDS, POS_SIGNALS, NEG_SIGNALS, RESPONSES, NJ_QUESTIONS,
} from '../lib/data';
    ...
    <div style={{ minHeight: '100vh', display: 'flex' }}>
      ...
      <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '26px 28px 60px', flex: 1 }}>
          {screen === 'overview' && <Overview go={go} setModal={setModal} />}
          {screen === 'dept' && <Dept dept={dept} filter={filter} setFilter={setFilter} setModal={setModal} />}
          {screen === 'papip' && <PaPip filter={filter} setFilter={setFilter} setModal={setModal} />}
          {screen === 'worryindex' && <WorryIndex setModal={setModal} />}
...
function Overview({ go, setModal }) {
  ...
  const reviewQueue = EMP.slice(0, 5).map(decorate);
  const paPipList = EMP.filter((e) => e.status !== 'In Progress').map((e) => ({ name: e.name, due: e.due, type: e.status === 'PIP Issued' ? 'PIP' : 'PA', ...STATUS[e.status] }));
...
function Dept({ dept, filter, setFilter, setModal }) {
  const deptEmp = EMP.filter((e) => e.team === dept);
  const pool = deptEmp.length ? deptEmp : EMP;
...
function PaPip({ filter, setFilter, setModal }) {
  ...
  const rows = EMP.filter((e) => e.status !== 'In Progress').filter((e) => !filter || e.team === filter).map(decorate);
...
function WorryIndex({ setModal }) {
  const ranked = EMP.map(decorate).sort((a, b) => a.score - b.score);
...
function Reports() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18, position: 'relative' }}>
      ...
      {report15 && <Report15Modal dept={report15} onClose={() => setReport15(null)} />}
      <div style={{ ...card, overflow: 'hidden' }}>
          ...
          return (
            <div key={r.name} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
              <div className={canExpand ? 'hoverrow' : ''} onClick={() => canExpand && setExpanded(open ? null : r.name)} style={{ display: 'grid', gridTemplateColumns: '1.3fr .8fr .8fr 1fr 1fr', padding: '13px 18px', alignItems: 'center', fontSize: 13, cursor: canExpand ? 'pointer' : 'default' }}>
                ...
                <span style={{ justifySelf: 'start', fontSize: 10.5, padding: '4px 9px', borderRadius: 999, background: r.bg, color: r.color, border: `1px solid ${r.border}` }}>{r.state}</span>
                <span style={{ textAlign: 'right', fontSize: 12, color: '#A8AEC4' }}>AI rating: <span style={{ color: r.color }}>{r.ai}</span>{canExpand && <span style={{ marginLeft: 8, color: '#6E7488' }}>{open ? '▲' : '▼'}</span>}</span>
              </div>
              {open && canExpand && (
...
function Report15Modal({ dept, onClose }) {
  const cards = CARD_DEFS[dept] || CARD_DEFS.Sales;
  const emp = EMP.filter((e) => e.team === dept).map(decorate);
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(4,6,12,0.72)', backdropFilter: 'blur(6px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40, zIndex: 60 }}>
