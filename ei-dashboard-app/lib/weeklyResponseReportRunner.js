...
// Runs mid-week (Tuesday) against the same ISO week Monday's weekly check-in
// email went out for, so it reports on responses received so far — not a
// closed-out week. Every active NJ appears exactly once, whether they
...
export async function sendWeeklyResponseReport() {
  ...
  return { message: `Sent weekly response report for ${week}: ${received} of ${total} active NJs responded, ${missed.length} missed.` };
}
